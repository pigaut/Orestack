...
public class StatRemoveHealthSubCommand extends SubCommand {

    public StatRemoveHealthSubCommand(@NotNull EnhancedPlugin plugin) {
        super(plugin, "remove-health");
        withPermission(plugin.getPermission("stat.remove-health"));
        withDescription(plugin.getTranslation("stat-remove-health-command"));
        ...
        withCommandExecution((sender, context, args) -> {
            ...
            plugin.sendMessage(sender, context, "removed-health");
        });
    }
