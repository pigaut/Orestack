...
public class AuraSkillsHook {
    ...
    public static void registerAllConditions(@NotNull EnhancedPlugin plugin) {
        ...
        if (!Server.isPluginEnabled("AuraSkills")) {
            conditions.register("HAS_AURA_LEVEL", NOT_ENABLED_CONDITION_LOADER);
            conditions.register("HAS_AURA_MANA", NOT_ENABLED_CONDITION_LOADER);
            return;
        }

        conditions.register("HAS_AURA_LEVEL", (ConfigLoader.Line<Condition>) line ->
                new HasAuraSkillLevel(
                        line.getRequired(1, Amount.class),
                        line.get("skill", Skills.class).withDefault(Skills.MINING)
                ));

        conditions.register("HAS_AURA_MANA", (ConfigLoader.Line<Condition>) line ->
    ...
    public static void registerAllActions(@NotNull EnhancedPlugin plugin) {
        ...
        if (!Server.isPluginEnabled("AuraSkills")) {
            actions.register("GIVE_AURA_EXP", NOT_ENABLED_ACTION_LOADER);
            actions.register("GIVE_AURA_MANA", NOT_ENABLED_ACTION_LOADER);
            actions.register("TAKE_AURA_MANA", NOT_ENABLED_ACTION_LOADER);
            return;
        }

        actions.register("GIVE_AURA_EXP", (ConfigLoader.Line<Action>) line ->
                new GiveAuraExp(
                        line.getRequired(1, Amount.class),
                        line.get("skill", Skills.class).withDefault(Skills.MINING),
        ...
        actions.register("GIVE_AURA_MANA", (ConfigLoader.Line<Action>) line ->
                new GiveAuraMana(line.getRequired(1, Amount.class)));

        actions.register("TAKE_AURA_MANA", (ConfigLoader.Line<Action>) line ->
