...
public class StatRemoveManaSubCommand extends SubCommand {

    public StatRemoveManaSubCommand(@NotNull EnhancedPlugin plugin) {
        super(plugin, "remove-mana");
        withPermission(plugin.getPermission("stat.remove-mana"));
        withDescription(plugin.getTranslation("stat-remove-mana-command"));
        ...
        withCommandExecution((sender, context, args) -> {
            ...
            playerState.setMana(playerState.getMana() - amount);

            context.addPlaceholder("player", target.getName());
            plugin.sendMessage(sender, context, "removed-mana");
        });
    }
