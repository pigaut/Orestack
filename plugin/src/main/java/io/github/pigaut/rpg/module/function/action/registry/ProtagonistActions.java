...
public class ProtagonistActions {
    ...
    public static void registerAll(@NotNull EnhancedPlugin plugin) {
        ...
        actions.register("DAMAGE_ENEMY", (ConfigLoader.Line<Action>) line ->
                new ProtagonistAttackEnemy(plugin, line.get(1, Amount.class).withDefault(Amount.ONE)));

        actions.register("SPAWN_PARTICLE_AT_PROTAGONIST", (ConfigLoader.Line<Action>) line ->
                new SpawnParticleAtProtagonist(line.getRequired(1, ParticleEffect.class)));

        actions.register("PLAY_SOUND_AT_PROTAGONIST", (ConfigLoader.Line<Action>) line ->
                new PlaySoundAtProtagonist(line.getRequired(1, SoundEffect.class)));

        actions.register("ADD_PROTAGONIST_COOLDOWN", (ConfigLoader.Line<Action>) line ->
                new AddProtagonistCooldown(
                        line.getRequiredString(1),
                        line.getRequired("duration", Delay.class)
                ));

        actions.register("REMOVE_PROTAGONIST_COOLDOWN", (ConfigLoader.Line<Action>) line ->
                new RemoveProtagonistCooldown(line.getRequiredString(1)));

        actions.registerAlias("DAMAGE_ENEMY", "DAMAGE_ENTITY", "DAMAGE_TARGET", "DAMAGE_VICTIM");

        actions.registerAlias("SPAWN_PARTICLE_AT_PROTAGONIST", "SPAWN_PARTICLE");
        actions.registerAlias("PLAY_SOUND_AT_PROTAGONIST", "PLAY_SOUND");

        actions.registerAlias("ADD_PROTAGONIST_COOLDOWN", "ADD_COOLDOWN", "START_PROTAGONIST_COOLDOWN", "START_COOLDOWN");
        actions.registerAlias("REMOVE_PROTAGONIST_COOLDOWN", "REMOVE_COOLDOWN", "STOP_PROTAGONIST_COOLDOWN", "STOP_COOLDOWN");
